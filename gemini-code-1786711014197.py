                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.post(list_url, json=payload, headers=headers) as resp:
                            ...
                            error_detail += f", 状态码: {resp.status}, 响应: {resp_text[:100]}"
...
def extract_jav_prefix(name: str) -> Optional[str]:
    ...
    clean_name = re.sub(
        r'[\U0001F600-\U0001F64F\U0001F300-\U0001F5FF\U0001F680-\U0001F6FF\U0001F700-\U0001F77F\U0001F780-\U0001F7FF\U0001F800-\U0001F8FF\U0001F900-\U0001F9FF\U0001FA70-\U0001FAFF\U00002702-\U000027B0\U00002600-\U000026FF]+',
        '', name, flags=re.UNICODE)
    clean_name = re.sub(
        r'(?:[Cc]hinese|[Ee]nglish)\s*[Ss]ubtitles?|中文字幕|英文字幕|高清|无码|無碼|无水印|無水印|破解版|破解|[\(\[]\d{4}[\)\]]|'
        r'FHD|HD|SD|1080p|720p|4K|Blu[Rr]ay|WEB-DL|REPACK|UNCENSORED|LEAKED|'
        r'\[.*?\]|【.*?】|\(.*?\)',
        '',
        clean_name,
        flags=re.IGNORECASE
...
def main() -> None:
    ...
    application.add_handler(CallbackQueryHandler(setdir_callback, pattern=r"^setdir_\d+$"))
    ...
    if config.SIZE_THRESHOLD > 0 and config.ALIST_OFFLINE_DIRS:
        ...
        for user_id in config.ALLOWED_USER_IDS:
            if f"notify_task_{user_id}" not in application.bot_data:
                application.bot_data[f"notify_task_{user_id}"] = False
            if f"notify_clean_{user_id}" not in application.bot_data:
                application.bot_data[f"notify_clean_{user_id}"] = True
    logger.info("机器人启动...")
